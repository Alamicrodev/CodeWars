// The Mormons are trying to find new followers and in order to do that they
//  embark on missions.

// Each time they go on a mission, each Mormon converts a fixed number of people (reach)
//  into followers. This continues and every freshly converted Mormon as well as every original Mormon go on another mission and convert the same fixed number of people each. The process continues until they reach a predefined target number of followers (target).
...
// PREP
// P: startingNum, reach, target
// R: number of missions
// E: 5, 3, 15 => 1
// P

function Mormons(startingNumber, reach, target) {
    // initiate missions variable at 0
    let missions = 0
    //while loop startingNumber < target
    while(startingNumber < target) {
      // increment missions
      // newconverts = startingNumber * reach
      // startingNumber += NewConverts
      missions++;
      let newConverts = startingNumber * reach
      startingNumber += newConverts
    }
    // return missions
    return missions
